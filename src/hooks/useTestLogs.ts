...
export function useTestLogs() {
  ...
  const clearLogs = useCallback(async () => {
    setLogs([]);
    await supabase.from("test_logs").delete().neq("id", "00000000-0000-0000-0000-000000000000");
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "test_logs" },
        (payload) => {
      ...
      .on(
        "postgres_changes",
        { event: "DELETE", schema: "public", table: "test_logs" },
        () => {
